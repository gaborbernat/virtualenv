...
packaging checks that already run on every pull request. Also add ``metadata.timestamp`` (derived from
``SOURCE_DATE_EPOCH`` for the same reproducibility) and ``metadata.tools`` naming the generator.

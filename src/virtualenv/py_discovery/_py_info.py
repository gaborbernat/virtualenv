...
class PythonInfo:
    ...
    def __init__(self) -> None:
        ...
        # pointer size.
        self.architecture = 32 if struct.calcsize("P") == 4 else 64
        ...
        confs = {
            k: (
                self.system_prefix
                if isinstance(v, str) and v.startswith(self.prefix)
                else v
            )
    ...
    def _fast_get_system_executable(self):
        ...
        if not (
            self.real_prefix
            or (self.base_prefix is not None and self.base_prefix != self.prefix)
        ):
    ...
    def sysconfig_path(
        self, key: str, config_var: dict[str, str] | None = None, sep: str = os.sep
    ) -> str:
    ...
    def clear_cache(cls, cache: PyInfoCache) -> None:
        """Clear all cached interpreter information from the cache.

        :param cache: the cache to clear

        """
        from ._cached_py_info import clear

        clear(cache)
        cls._cache_exe_discovery.clear()

    def satisfies(self, spec: PythonSpec, impl_must_match: bool) -> bool:
    ...
    def current(cls, cache: PyInfoCache | None = None) -> PythonInfo:
        ...
        :param cache: the cache for interpreter information
        ...
        if cls._current is None:
            result = cls.from_exe(
                sys.executable, cache, raise_on_error=True, resolve_to_host=False
            )
            if result is None:
    ...
    def current_system(cls, cache: PyInfoCache | None = None) -> PythonInfo:
        ...
        if cls._current_system is None:
            result = cls.from_exe(
                sys.executable, cache, raise_on_error=True, resolve_to_host=True
            )
            if result is None:
    ...
    def _to_dict(self):
        ...
        data["version_info"] = (
            version_info._asdict() if hasattr(version_info, "_asdict") else version_info
        )
        return data
    ...
    def from_exe(
        ...
        """
        from ._cached_py_info import from_exe
        ...
        if isinstance(proposed, PythonInfo) and resolve_to_host:
            try:
                proposed = proposed._resolve_to_system(cache, proposed)
    ...
    @classmethod
    def _resolve_to_system(cls, cache, target):
    ...
    def discover_exe(
        ...
        for folder in possible_folders:
            for name in possible_names:
                info = self._check_exe(cache, folder, name, exact, discovered, env)
                if info is not None:
    ...
    def _check_exe(self, cache, folder, name, exact, discovered, env):
        exe_path = os.path.join(folder, name)
        if not os.path.exists(exe_path):
            return None
        info = self.from_exe(
            exe_path, cache, resolve_to_host=False, raise_on_error=False, env=env
        )

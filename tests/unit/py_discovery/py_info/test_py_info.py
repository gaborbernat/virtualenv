...
from virtualenv.py_discovery import DiskCache, PythonInfo, PythonSpec
...
def test_py_info_cached_error(mocker, tmp_path: Path, session_cache: DiskCache) -> None:
...
def test_py_info_cache_clear(mocker, session_cache: DiskCache) -> None:
...
def test_system_executable_no_exact_match(
    ...
    def func(k, cache, resolve_to_host, raise_on_error, env):
    ...
    mapped = target_py_info._resolve_to_system(session_cache, target_py_info)
...
def test_py_info_machine_derivation_universal2(mocker, runtime_isa: str) -> None:
    info = copy.deepcopy(CURRENT)

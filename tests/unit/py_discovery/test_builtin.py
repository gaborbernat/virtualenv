...
from virtualenv.py_discovery import Builtin, DiskCache, PythonInfo, get_interpreter
from virtualenv.py_discovery._builtin import LazyPathDump, get_paths


@pytest.mark.graalpy
@pytest.mark.skipif(not os.path.islink(sys.executable) and not os.path.isfile(sys.executable), reason="no symlink")
...
def test_uv_python(monkeypatch: pytest.MonkeyPatch, tmp_path_factory: pytest.TempPathFactory, mocker) -> None:
...
def test_discovery_fallback_fail(session_cache: DiskCache, caplog: pytest.LogCaptureFixture) -> None:
    caplog.set_level(logging.DEBUG)
    builtin = Builtin(python_spec=["magic-one", "magic-two"], cache=session_cache)
...
def test_discovery_fallback_ok(session_cache: DiskCache, caplog: pytest.LogCaptureFixture) -> None:
    caplog.set_level(logging.DEBUG)
    builtin = Builtin(python_spec=["magic-one", sys.executable], cache=session_cache)
...
@pytest.fixture
def mock_get_interpreter(mocker) -> None:
    mocker.patch(
        "virtualenv.py_discovery._builtin.get_interpreter",
        lambda key, *args, **kwargs: getattr(mocker.sentinel, key),
    )


@pytest.mark.usefixtures("mock_get_interpreter")
def test_returns_first_python_specified(mocker, session_cache: DiskCache) -> None:
    builtin = Builtin(python_spec=["python_from_cli"], cache=session_cache)
    result = builtin.run()
